...
public class StockDealRankScheduler {
    ...
    public void init() {
        //시작 로그 출력
        log.debug("Start StockDealRankScheduler");
    }
    ...
    public void job() throws InterruptedException {
        ...
        try {
    		...
    		for (Element tr: tbody) {
    			...
    			String cd    = tds.eq(0).get(0).select("a").attr("href").replaceAll("/item/main.nhn\\?code=", "");  // 종목코드
    			String nm    = tds.eq(0).get(0).select("a").text();    // 종목명
    			...
    			stockDealRankVo.setRank(String.valueOf(rank++));             // 순위
